...
if __name__ == '__main__':
    ...
    for seed in seed_list:
        ...
        top1_torch.update((prec1_torch.item(), data_torch.shape[0]))
        top5_torch.update(prec5_torch.item(), data_torch.shape[0])
        reprod_log_paddle.add(f"data_{seed_list.index(seed) + 1}_top1", top1_paddle.avg)
        reprod_log_paddle.add(f"data_{seed_list.index(seed) + 1}_top5", top5_paddle.avg)
        reprod_log_torch.add(f"data_{seed_list.index(seed) + 1}_top1", top1_torch.avg)
        reprod_log_torch.add(f"data_{seed_list.index(seed) + 1}_top5", top5_torch.avg)
